...
function _JS_DeleteFile(offlinePath, fileID, listenerID) {
 var jFileID = UTF8ToString(fileID);
 var jOfflinePath = UTF8ToString(offlinePath);
 var jlistenerID = UTF8ToString(listenerID);
 var db;
 OpenDB = function() {
  ...
  var request = indexedDB.open("YourDB", 1);
  ...
  request.onsuccess = function(event) {
   ...
   db = request.result;
   if (!db.objectStoreNames.contains("TWXRFiles")) {
    console.log("No video objectstorename, Deleting the current database");
    indexedDB.deleteDatabase("YourDB");
    var JsonCallBack = JSON.stringify({
     Error: "No video objectstorename, Deleted the current database",
     OfflinePath: jOfflinePath,
     FileID: jFileID,
     ListenerID: jlistenerID
    });
    SendMessage("WebListener", "Player_DeleteProject_Listener", JsonCallBack);
    return;
   } else {
    window.onload = deleteFileFromDb();
   }
   db.onerror = function(event) {
    console.log("Error creating/accessing IndexedDB database");
    var JsonCallBack = JSON.stringify({
     Error: "Error creating/accessing IndexedDB database",
     OfflinePath: jOfflinePath,
     FileID: jFileID,
     ListenerID: jlistenerID
    });
  ...
  request.onupgradeneeded = function(event) {
   console.log("event.old", event.oldVersion, "event.new", event.newVersion);
   if (!event.target.result.objectStoreNames.contains("TWXRFiles")) {
    event.target.result.createObjectStore("TWXRFiles");
    console.log("Created TWXRFiles object store");
   }
  };
 };
 OpenDB();
 deleteFileFromDb = function() {
  console.log("deleteFileFromDb" + jFileID);
  var transaction = db.transaction([ "TWXRFiles" ], "readwrite");
  transaction.onerror = function(event) {
   ...
   SendMessage("WebListener", "Player_DeleteProject_Listener", JsonCallBack);
   console.log("Error transaction database" + event.target.errorCode);
   return;
  };
  transaction.objectStore("TWXRFiles").get(jFileID).onsuccess = function(event) {
   var URL = window.URL || window.webkitURL;
   try {
    videoURL = (window.URL ? URL : webkitURL).revokeObjectURL(event.target.result["Blob"]);
    var del = transaction.objectStore("TWXRFiles").delete(jFileID);
    del.onsuccess = function() {
     console.log("success deleted the file" + jFileID);
     var JsonCallBack = JSON.stringify({
      OfflinePath: jOfflinePath,
      FileID: jFileID,
      ListenerID: jlistenerID
    ...
    del.onerror = function(event) {
     ...
     SendMessage("WebListener", "Player_DeleteProject_Listener", JsonCallBack);
     console.log("Error putting database" + event.target.errorCode);
    };
   } catch (e) {
    alert("Your Browser doesnt support createObjectURL!" + e);
   }
  };
 };
}
...
function _JS_GetAllFileListAlreadyDownloadedInDB(listenerID) {
 var jlistenerID = UTF8ToString(listenerID);
 window.indexedDB = window.indexedDB || window.webkitIndexedDB || window.mozIndexedDB || window.OIndexedDB || window.msIndexedDB, 
 IDBTransaction = window.IDBTransaction || window.webkitIDBTransaction || window.OIDBTransaction || window.msIDBTransaction, 
 dbVersion = 1;
 ...
 request.onsuccess = function(event) {
  var ListOfFileListAlreadyDownloadedInDB = new Array();
  if (!request.result.objectStoreNames.contains("TWXRFiles")) {
   var JsonCallBack = JSON.stringify({
    FilesOnDevice: ListOfFileListAlreadyDownloadedInDB,
    ListenerID: jlistenerID
   });
   SendMessage("WebListener", "GetAllFileListAlreadyDownloadedInDBListener", JsonCallBack);
   return;
  }
  var transaction = request.result.transaction([ "TWXRFiles" ], "readwrite");
  var objectStore = transaction.objectStore("TWXRFiles");
  objectStore.openKeyCursor().onsuccess = function(event) {
   var cursor = event.target.result;
   if (cursor) {
    objectStore.get(cursor.key).onsuccess = function(event) {
     var URL = window.URL || window.webkitURL;
     try {
      videoURL = (window.URL ? URL : webkitURL).createObjectURL(event.target.result["Blob"]);
      console.log("videoURL " + videoURL);
      ListOfFileListAlreadyDownloadedInDB.push({
       FileID: cursor.key,
       OfflinePath: videoURL
      });
      cursor.continue();
     } catch (e) {
      alert("Your Browser doesnt support createObjectURL!" + e);
...
function _JS_GetUrlFromIndexedDB(fileID, listenerID) {
 return new Promise(function(resolve) {
  var jFileID = UTF8ToString(fileID);
  var jlistenerID = UTF8ToString(listenerID);
  console.log("jFileID : " + jFileID);
  window.indexedDB = window.indexedDB || window.webkitIndexedDB || window.mozIndexedDB || window.OIndexedDB || window.msIndexedDB, 
  IDBTransaction = window.IDBTransaction || window.webkitIDBTransaction || window.OIDBTransaction || window.msIDBTransaction, 
  dbVersion = 1;
  if (!window.indexedDB) {
  ...
  request.onsuccess = function(event) {
   if (!request.result.objectStoreNames.contains("TWXRFiles")) {
    ...
    SendMessage("WebListener", "GetUrlFromIndexedDBListener", JsonCallBack);
    return;
   }
   var transaction = request.result.transaction([ "TWXRFiles" ], "readwrite");
   transaction.objectStore("TWXRFiles").get(jFileID).onsuccess = function(event) {
    ...
    try {
     ...
     videoURL = "";
     var JsonCallBack = JSON.stringify({
      Error: "Your Browser doesnt support createObjectURL",
      FileID: jFileID,
      OfflinePath: videoURL,
      ListenerID: jlistenerID
     });
...
function _JS_SaveVideoInIndexedDB(onlinePath, fileID, listenerID) {
 var jOnlinePath = UTF8ToString(onlinePath);
 var jFileID = UTF8ToString(fileID);
 var jlistenerID = UTF8ToString(listenerID);
 var db;
 OpenDB = function() {
  ...
  request.onsuccess = function(event) {
   ...
   if (!db.objectStoreNames.contains("TWXRFiles")) {
    ...
    var JsonCallBack = JSON.stringify({
     Error: "No video objectstorename, Deleted the current database",
     OnlinePath: jOnlinePath,
     FileID: jFileID,
     ListenerID: jlistenerID
    });
    SendMessage("WebListener", "SaveVideoInIndexedDBListener", JsonCallBack);
    return;
   } else {
 ...
 getVideoFile = function() {
  ...
  var blob;
  xhr.open("GET", jOnlinePath, true);
  xhr.responseType = "blob";
  xhr.addEventListener("load", function() {
   if (xhr.status === 200) {
    blob = xhr.response;
    console.log("SUCCESS: Video file downloaded " + jFileID);
    putVideoInDb(blob);
   } else {
    console.log("ERROR: Unable to download video." + xhr.status);
    var JsonCallBack = JSON.stringify({
  ...
  xhr.addEventListener("progress", function(event) {
   if (event.lengthComputable && event.total > 0) {
    ...
    if (complete != 0 && complete != 100) {
     ...
     SendMessage("WebListener", "SaveVideoInIndexedDBListener_Progress", JsonCallBack);
    }
   }
  });
  xhr.send();
 }, putVideoInDb = function(blob) {
  console.log("Openning the transaction");
  var transaction = db.transaction([ "TWXRFiles" ], "readwrite");
  console.log("Setting video properties");
  transaction.onerror = function(event) {
   console.log("Error transaction database");
   console.log("Error transaction database" + event.target.errorCode);
  };
  var VideoProperties = {
   Blob: blob,
  ...
  var put = transaction.objectStore("TWXRFiles").put(VideoProperties, jFileID);
  put.onerror = function(event) {
   console.log("Error putting database");
   console.log("Error putting database" + event.target.errorCode);
  };
  console.log("Video Put on the database");
  transaction.objectStore("TWXRFiles").get(jFileID).onsuccess = function(event) {
...
var asmLibraryArg = {
 ...
 "JS_DeleteFile": _JS_DeleteFile,
 "JS_Eval_OpenURL": _JS_Eval_OpenURL,
 "JS_FileSystem_Initialize": _JS_FileSystem_Initialize,
 "JS_FileSystem_Sync": _JS_FileSystem_Sync,
 "JS_GetAllFileListAlreadyDownloadedInDB": _JS_GetAllFileListAlreadyDownloadedInDB,
 "JS_GetUrlFromIndexedDB": _JS_GetUrlFromIndexedDB,
 ...
 "JS_SaveVideoInIndexedDB": _JS_SaveVideoInIndexedDB,
 ...
 "JS_SetCancelled": _JS_SetCancelled,
...
var dynCall_ifffiiiiiiiiii = Module["dynCall_ifffiiiiiiiiii"] = createExportWrapper("dynCall_ifffiiiiiiiiii");

var dynCall_vfffffffiiii = Module["dynCall_vfffffffiiii"] = createExportWrapper("dynCall_vfffffffiiii");
